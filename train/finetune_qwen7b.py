"""Train an isolated Qwen2.5 7B QLoRA without touching 1.5B or 3B artifacts."""
...
MODEL_PATH = PROJECT_DIR / "models" / "qwen2.5-7b-instruct-4bit"
DATASET_PATH = PROJECT_DIR / "data" / "mixed_dataset_v2.jsonl"
DATA_DIR = PROJECT_DIR / "data" / "qwen7b"
TRAIN_PATH = DATA_DIR / "train.jsonl"
VALID_PATH = DATA_DIR / "valid.jsonl"
ADAPTER_PATH = PROJECT_DIR / "models" / "qwen2.5-7b-synapse-lora-v1"
CONFIG_PATH = PROJECT_DIR / "train" / "configs" / "qwen7b_lora.yaml"
TRAIN_SPLIT = 0.9
SPLIT_SEED = 42
SYSTEM_PROMPT = (
...
def train() -> None:
    if not model_is_complete(MODEL_PATH):
        raise SystemExit(
            ...
            "Run: ./venv/bin/python scripts/prepare_qwen7b_mlx.py"
        )
    ...
    print("Qwen2.5 7B QLoRA configuration")
    print(f"Base: {MODEL_PATH}")
    print(f"Dataset: {DATASET_PATH}")
    print(f"Train/valid: {train_count}/{valid_count}")
    print(f"Adapters: {ADAPTER_PATH}")
    print("Config tuning: 6 LoRA layers, max_seq_length=768, grad_checkpoint=true")
    if checkpoint is not None:
        print(f"Resuming: {checkpoint}")
    print("Running:")
    print(" ".join(command))
    subprocess.run(command, cwd=PROJECT_DIR, check=True)
    print(f"Qwen2.5 7B adapters ready: {ADAPTER_PATH}")
