"""Fuse the Qwen2.5 7B LoRA and export a Q4_K_M GGUF for Ollama."""
...
BASE_MODEL_PATH = PROJECT_DIR / "models" / "qwen2.5-7b-instruct-4bit"
ADAPTER_PATH = PROJECT_DIR / "models" / "qwen2.5-7b-synapse-lora-v1"
MERGED_PATH = PROJECT_DIR / "models" / "qwen2.5-7b-synapse-merged-v1"
OUTPUT_DIR = PROJECT_DIR / "models" / "gguf"
F16_GGUF_PATH = OUTPUT_DIR / "synapse-qwen2.5-7b-v1-f16.gguf"
Q4_GGUF_PATH = OUTPUT_DIR / "synapse-qwen2.5-7b-v1-q4_k_m.gguf"
...
def validate() -> None:
    if not (BASE_MODEL_PATH / "config.json").exists() or not any(
        ...
        raise SystemExit(f"Complete Qwen2.5 7B MLX base not found: {BASE_MODEL_PATH}")
    if not (ADAPTER_PATH / "adapters.safetensors").exists():
        raise SystemExit(f"Final Qwen2.5 7B adapters not found: {ADAPTER_PATH}")
...
def main() -> None:
    ...
    if not (MERGED_PATH / "config.json").exists():
        run(
            ...
            "1/3 Fusing Qwen2.5 7B adapters",
        )
    else:
    ...
    if not F16_GGUF_PATH.exists():
        run(
            ...
            "2/3 Converting Qwen2.5 7B to F16 GGUF",
            env=converter_env(),
        )
    else:
    ...
    if not Q4_GGUF_PATH.exists():
        run(
            ...
            "3/3 Quantizing Qwen2.5 7B GGUF",
        )
    else:
        print(f"\n3/3 Reusing Q4_K_M GGUF: {Q4_GGUF_PATH}")

    print(f"\nQwen2.5 7B GGUF ready: {Q4_GGUF_PATH}")
    print("Register it with:")
    print("  ollama create synapse-qwen2.5-7b-v1 -f Qwen7BSynapseModelfile")
